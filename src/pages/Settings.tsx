...
export const Settings: React.FC = () => {
  ...
  useEffect(() => {
    if (state.user) {
      setProfileData({
        ...
        phone: '', // Phone não está no User type, mas pode ser adicionado
      });
    }
  }, [state.user]);
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authToken === 'demo-token' || profileData.email === 'test@example.com') {
        // Simulate API delay
        ...
        showSuccess('Perfil atualizado!', 'Suas informações foram salvas com sucesso (modo teste)');
        setProfileErrors({});
        return;
      }
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Check for test credentials
      const authToken = localStorage.getItem('authToken');
      if (authToken === 'demo-token' || state.user?.email === 'test@example.com') {
        // Simulate API delay
        await new Promise(resolve => setTimeout(resolve, 1500));
        
        // Mock password validation - accept "123456" as current password for demo
        if (passwordData.currentPassword !== '123456') {
          setPasswordErrors({ currentPassword: 'Senha atual incorreta (use: 123456 para teste)' });
          showError('Senha incorreta', 'Para o modo teste, use "123456" como senha atual');
          return;
        }
        
        // Mock successful password change
        showSuccess('Senha alterada!', 'Sua senha foi alterada com sucesso (modo teste)');
        setPasswordData({
          currentPassword: '',
          newPassword: '',
          confirmPassword: '',

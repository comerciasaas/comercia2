...
class AIService {
  ...
  async searchKnowledge(userId, query, limit = 5) {
    const { executeUserQuery } = require('../config/database');
  ...
  async generateWithRAG(userId, provider, model, message, systemPrompt, temperature, maxTokens) {
    try {
      // Buscar conhecimento relevante
      const knowledge = await this.searchKnowledge(userId, message);
      
      let enhancedPrompt = systemPrompt || 'Você é um assistente útil.';
      ...
      return await this.sendMessage(provider, model, message, systemPrompt, temperature, maxTokens);
    }
  }
}
